...
namespace Machly.Api.GraphQL.Types
{
    public class MachineType : ObjectType<MachineDto>
    {
        protected override void Configure(IObjectTypeDescriptor<MachineDto> descriptor)
        {
            ...
            descriptor.Field(m => m.TariffsAgro).Type<TariffAgroType>();
        }
    }
    ...
    public class TariffAgroType : ObjectType<TariffAgroDto>
    {
        protected override void Configure(IObjectTypeDescriptor<TariffAgroDto> descriptor)
        {
            descriptor.BindFieldsExplicitly();
            descriptor.Field(t => t.Hectarea).Type<DecimalType>();
            descriptor.Field(t => t.Tonelada).Type<DecimalType>();
            descriptor.Field(t => t.KmTariffs).Type<ListType<KmTariffType>>();
        }
    }

    public class KmTariffType : ObjectType<KmTariffDto>
    {
        protected override void Configure(IObjectTypeDescriptor<KmTariffDto> descriptor)
        {
            descriptor.BindFieldsExplicitly();
            descriptor.Field(k => k.MinKm).Type<NonNullType<IntType>>();
            descriptor.Field(k => k.MaxKm).Type<NonNullType<IntType>>();
            descriptor.Field(k => k.TarifaPorKm).Type<NonNullType<DecimalType>>();
        }
    }
}

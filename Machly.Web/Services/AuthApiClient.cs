...
namespace Machly.Web.Services
{
    public class AuthApiClient
    {
        ...
        public async Task<bool> LoginAsync(string email, string password)
        {
            ...
            {
                new Claim(ClaimTypes.NameIdentifier, jwt.Claims.FirstOrDefault(c => c.Type == "id")?.Value ?? jwt.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier)?.Value ?? ""),
                new Claim(ClaimTypes.Email,       jwt.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Email)?.Value ?? jwt.Claims.FirstOrDefault(c => c.Type == "email")?.Value ?? ""),
                new Claim(ClaimTypes.Role,        jwt.Claims.FirstOrDefault(c => c.Type == ClaimTypes.Role)?.Value ?? jwt.Claims.FirstOrDefault(c => c.Type == "role")?.Value ?? ""),

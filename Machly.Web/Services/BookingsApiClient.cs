...
using Machly.Web.Models.ViewModels;
...
namespace Machly.Web.Services
{
    public class BookingsApiClient
    {
        ...
        {
            // Deprecated or keep for backward compatibility if used elsewhere
            return await CreateBookingAsync(new CreateBookingViewModel 
            { 
                MachineId = machineId, 
                StartDate = start, 
                EndDate = end 
            }) ? new Booking() : null; // Simplified return for compatibility
        }
        ...
        public async Task<bool> CreateBookingAsync(CreateBookingViewModel model)
        {
            var request = new
            {
                ...
                renterId = model.RenterId, // Send explicit ID
                start = model.StartDate,
                end = model.EndDate,
                hectareas = model.Hectareas,
